...
namespace Tests
{
    public class BookRepositoryTests
    {
        ...
        public async Task RemoveBookById_ReturnsBook_WhenRemovalIsSuccessful()
        {
            ...
            var bookToRemove = new Book { Id = bookId };
            _mockContext.Setup(c => c.Books.FindAsync(bookId)).ReturnsAsync(bookToRemove);

            // Act
            var result = await _bookRepository.RemoveBookById(bookId);

            // Assert
            Assert.Equal(bookToRemove, result);
        }*/
    }
}